<x-app-layout>
    <div>
        ...
        <h2 class="text-xl font-medium mt-3">Now choose a s ervice from {{ $employee->name }}</h2>
        <div class="grid grid-cols-2 md:grid-cols-5 gap-8 mt-6">
            @foreach ($services as $service)
                <a href="{{ route('checkout', [$employee, $service]) }}"
                    ...
                    <div class="text-xs mt-3 text-slate-600 bg-slate-200 rounded-lg py-0.5 px-1.5">{{ $service->price }}
                    </div>
                </a>
            @endforeach
        </div>
